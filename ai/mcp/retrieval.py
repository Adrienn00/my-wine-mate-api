...
def build_recipe_candidates_via_search_plan(
    ...
    try:
        search_plan = parse_search_spec(
            call_llm(build_wine_to_recipe_search_prompt(wine, preferences), system_prompt=SEARCH_PLAN_SYSTEM_PROMPT, model_override=RECOMMEND_MODEL)
        )
    except Exception:
        pass
...
def build_wine_candidates_via_search_plan(
    ...
    try:
        search_plan = parse_search_spec(
            call_llm(build_recipe_to_wine_search_prompt(recipe, preferences), system_prompt=SEARCH_PLAN_SYSTEM_PROMPT, model_override=RECOMMEND_MODEL)
        )
    except Exception:
        pass
    planning_ms = round((time.perf_counter() - planning_started_at) * 1000, 2)
